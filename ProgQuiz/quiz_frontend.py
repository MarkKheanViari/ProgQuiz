...
def check_score():
    ...
    if len(completed_intensities) == len(quiz_backend.intensity_data):
        ...
        score_message = "\n".join([f"{intensity}: {user_scores.get(intensity, 'Not completed')}/{len(quiz_backend.intensity_data[intensity])}" for intensity in quiz_backend.intensity_data.keys()])
...
# Change the window logo using the iconbitmap method
